...
%% API
-export([calc_linear_least_squares/1, calc_exp_least_squares/1, calc_power_least_squares/1, calc_arguments/2]).

calc_linear_least_squares(Points) when length(Points) > 1 ->
  PartialResult = #linap_partial{sx = 0, sxx = 0, sy = 0, sxy = 0, number_of_points = length(Points)},
  calc_linear_least_squares(Points, PartialResult).

calc_linear_least_squares([], #linap_partial{sx = SX, sxx = SXX, sy = SY, sxy = SXY, number_of_points = NumberOfPoints}) ->
...
calc_linear_least_squares([Point | Tail], #linap_partial{sx = SX, sxx = SXX, sy = SY, sxy = SXY, number_of_points = NumberOfPoints}) ->
  #point{x = X, y = Y} = Point,
  calc_linear_least_squares(Tail, #linap_partial{sx = SX + X, sxx = SXX + X * X, sy = SY + Y, sxy = SXY + X * Y, number_of_points = NumberOfPoints}).


calc_exp_least_squares(Points) when length(Points) > 1 ->
  PartialResult = #expap_partial{sx = 0, sxx = 0, sy = 0, sxy = 0, number_of_points = length(Points)},
  calc_exp_least_squares(Points, PartialResult).

calc_exp_least_squares([], #expap_partial{sx = SX, sxx = SXX, sy = SY, sxy = SXY, number_of_points = NumberOfPoints}) ->
  Determinant = SXX * NumberOfPoints - SX * SX,
  case Determinant of
    0 -> not_possible;
    _ ->
...
calc_exp_least_squares([Point | Tail], #expap_partial{sx = SX, sxx = SXX, sy = SY, sxy = SXY, number_of_points = NumberOfPoints}) ->
  #point{x = X, y = Y} = Point,
  case Y =< 0 of
    true -> not_possible;
    _ ->
      calc_exp_least_squares(Tail, #expap_partial{
        sx = SX + X,
        sxx = SXX + X * X,
        sy = SY + math:log(Y),
...
calc_power_least_squares(Points) when length(Points) > 1 ->
  PartialResult = #powap_partial{sx = 0, sxx = 0, sy = 0, sxy = 0, number_of_points = length(Points)},
  calc_power_least_squares(Points, PartialResult).

calc_power_least_squares([], #powap_partial{sx = SX, sxx = SXX, sy = SY, sxy = SXY, number_of_points = NumberOfPoints}) ->
  Determinant = SXX * NumberOfPoints - SX * SX,
  case Determinant of
    0 -> not_possible;
    _ ->
      ...
      fun(X) ->
        case X < 0 of
          true -> undef;
          _ -> CoefA * math:pow(X, CoefB)
        end
      end
  end;
calc_power_least_squares([Point | Tail], #powap_partial{sx = SX, sxx = SXX, sy = SY, sxy = SXY, number_of_points = NumberOfPoints}) ->
  #point{x = X, y = Y} = Point,
  case {X =< 0, Y =< 0} of
    {true, _} -> not_possible;
    {_, true} -> not_possible;
    _ ->
      calc_power_least_squares(Tail, #powap_partial{
        sx = SX + math:log(X),
...
calc_arguments(ResultPoints, _, []) -> ResultPoints;
